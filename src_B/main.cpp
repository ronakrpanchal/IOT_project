...
const int LOCAL_PORT = 4211; // Listen on a *different* local port
...
// This is our SERVER logic: handles packets from Device A (via relay)
void data_callback(CoapPacket &packet, IPAddress ip, int port)
{
    ...
    // 2. Send an ACK back to the sender (Device A)
...
void setup()
{
    Serial.begin(115200);
    delay(1000);
    Serial.println("--- Starting Device B (CoAP) ---");
    ...
    coap.put(serverIp, SERVER_PORT, "reg", "REG-B");
